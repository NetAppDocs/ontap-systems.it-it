...
summary: 'Sostituisci il controller nel tuo sistema AFF A1K quando un guasto hardware lo richiede. Il processo di sostituzione prevede la rimozione del controller dalla parte anteriore dello chassis, la rimozione delle unità di alimentazione (PSU), della NVRAM12, dei moduli I/O e dei moduli vuoti e del modulo di gestione del sistema, quindi l"installazione dello chassis sostitutivo e la reinstallazione dei componenti dello chassis.' 
---
= Sostituire il controller - AFF A1K
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sostituisci il controller del tuo sistema AFF A1K quando un guasto hardware lo richiede. Il processo di sostituzione prevede la rimozione del controller danneggiato, lo spostamento dei componenti sul controller sostitutivo, l'installazione del controller sostitutivo e il riavvio.
...
Dopo aver sostituito il controller AFF A1K danneggiato, è necessario link:controller-replace-system-config-restore-and-verify.html["ripristinare la configurazione del sistema"].
