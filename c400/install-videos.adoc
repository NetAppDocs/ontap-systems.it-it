---
permalink: c400/install-videos.html 
sidebar: sidebar 
keywords: aff c400, aff, c400, video, rack, cable, system, manager, guide, setup, initial, configuration, hardware, installation, cabling, configuration, hardware installation and cabling, perform end-to-end software configuration 
summary: Il seguente video mostra come installare e collegare il nuovo sistema. 
---
= Installa e configura il tuo sistema storage AFF C400 usando le istruzioni video
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Installa e configura il tuo sistema storage AFF C400 seguendo le istruzioni video che mostrano come installare e cablare il sistema.

.Animazione - istruzioni per l'installazione e la configurazione di AFF C400
video::8e392453-beac-4db7-8088-aff1005e1f90[panopto]
Se si dispone di una configurazione MetroCluster, utilizzare il contenuto dell'installazione di MetroCluster.

https://docs.netapp.com/us-en/ontap-metrocluster/index.html["Documentazione MetroCluster"^]
