...
summary: È necessario sostituire il modulo NVDIMM nel controller quando il sistema rileva che la durata della memoria flash sta per terminare o che il modulo NVDIMM identificato non è in buono stato in generale, oppure la mancata sostituzione provoca un panic di sistema. 
---
= Sostituisci un modulo NVDIMM nel sistema storage AFF C400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sostituisci un NVDIMM nel tuo sistema storage AFF C400 quando il sistema registra un numero eccessivo di errori ECC correggibili o un errore ECC non correggibile, che di solito è causato dal guasto di un singolo NVDIMM.
...
image::../media/drw_A400_Replace-NVDIMM-DIMM_IEOPS-1009.svg[Sostituire l'NVDIMM]
