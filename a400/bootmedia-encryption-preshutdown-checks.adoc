...
---
= Verifica il supporto di crittografia sul supporto di avvio nel tuo sistema AFF A400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Per garantire la sicurezza dei dati sul sistema storage AFF A400, è necessario verificare il supporto e lo stato della chiave di crittografia sul supporto di avvio. Verificare se la versione di ONTAP supporta NetApp Volume Encryption (NVE) e, prima di spegnere il controller, verificare che il key manager sia attivo.
