...
sidebar: sidebar 
keywords: aff a400, aff, a400, shut, down, check, status, onboard, encryption, keys, status, disable, automatic, giveback, version, ontap, system, run, pre-shutdown, check, pre-shutdown checks for onboard encryption keys 
...
---
= Controlla il supporto della crittografia sul supporto di avvio in un AFF A400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Per mantenere la sicurezza dei dati sul sistema storage AFF A400, verifica il supporto e lo stato della chiave di crittografia sul supporto di boot prima di spegnere il controller. Controlla se la versione di ONTAP supporta NetApp Volume Encryption (NVE) e conferma che il key manager sia attivo.
