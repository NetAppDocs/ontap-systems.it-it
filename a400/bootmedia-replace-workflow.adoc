...
summary: 'Per iniziare a sostituire il supporto di avvio nel sistema di archiviazione AFF A400 , è necessario esaminare i requisiti di sostituzione, controllare lo stato della crittografia, spegnere il controller, sostituire il supporto di avvio, avviare l"immagine di ripristino, ripristinare la crittografia e verificare la funzionalità del sistema.' 
---
= Procedura di sostituzione del supporto di avvio per un sistema AFF A400 utilizzando il processo di ripristino manuale dell'avvio
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Inizia a sostituire il supporto di avvio nel tuo sistema AFF A400 utilizzando il processo di ripristino manuale dell'avvio. Devi scaricare l'immagine di ripristino ONTAP appropriata dal NetApp Support Site e copiarla su un'unità USB per eseguire il ripristino e riportare il sistema allo stato operativo.
