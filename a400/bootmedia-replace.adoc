...
summary: È necessario individuare il supporto di avvio nel modulo controller e quindi seguire le istruzioni per sostituirlo. 
---
= Sostituisci il supporto di avvio in un sistema AFF A400 utilizzando la procedura di ripristino manuale dell'avvio
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Il boot media nel tuo sistema AFF A400 memorizza dati essenziali di firmware e configurazione. La procedura di sostituzione prevede la rimozione del modulo controller, la rimozione del boot media danneggiato, l'installazione del boot media sostitutivo e quindi il trasferimento dell'immagine ONTAP sul boot media sostitutivo utilizzando una chiavetta USB.
