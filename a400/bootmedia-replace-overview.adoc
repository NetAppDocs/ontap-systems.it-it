...
summary: 'Il supporto di avvio memorizza un set primario e uno secondario di file di sistema che il sistema utilizza all"avvio.' 
---
= Consulta i requisiti per la sostituzione del supporto di avvio nel tuo sistema AFF A400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Prima di sostituire il boot media nel tuo sistema AFF A400, assicurati di soddisfare i requisiti necessari per una sostituzione riuscita. Questo include avere una unità flash USB formattata in FAT32 con spazio sufficiente per l'immagine di ripristino ONTAP e capire se effettuerai una sostituzione senza interruzioni o con interruzioni.
