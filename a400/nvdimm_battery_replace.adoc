...
. Al prompt DEL CARICATORE, accedere ai driver speciali appositamente progettati per la diagnostica per funzionare correttamente: `boot_diags`
. Selezionare *Scan System* (sistema di scansione) dal menu visualizzato per abilitare l'esecuzione dei test di diagnostica.
. Selezionare *Test Memory* dal menu visualizzato per eseguire i test di diagnostica.
