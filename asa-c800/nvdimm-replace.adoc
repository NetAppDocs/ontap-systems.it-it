...
summary: È necessario sostituire il modulo NVDIMM nel controller quando il sistema rileva che la durata della memoria flash sta per terminare o che il modulo NVDIMM identificato non è integro. In caso contrario, si verifica un panic di sistema. 
---
= Sostituisci un modulo NVDIMM nel tuo sistema ASA C800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sostituire la NVDIMM nel sistema ASA C800 quando il sistema rileva che la durata della flash è quasi giunta al termine o che la NVDIMM identificata non è in buone condizioni generali; in caso contrario, si verifica un errore del sistema.

.Prima di iniziare
* Assicurati di avere il modulo NVDIMM sostitutivo ricevuto da NetApp e compatibile con il tuo sistema ASA C800 .
