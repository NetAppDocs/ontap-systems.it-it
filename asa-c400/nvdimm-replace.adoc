...
summary: È necessario sostituire il modulo NVDIMM nel controller quando il sistema rileva che la durata della memoria flash sta per terminare o che il modulo NVDIMM identificato non è in buone condizioni generali. In caso contrario, si verifica un panic di sistema. 
---
= Sostituisci un modulo NVDIMM nel tuo sistema ASA C400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sostituisci un NVDIMM nel tuo sistema ASA C400 quando il sistema storage riscontra un numero eccessivo di errori ECC correggibili o errori ECC non correggibili, che sono in genere causati dal guasto di un singolo NVDIMM.
