...
summary: È necessario sostituire un modulo DIMM nel controller module quando il sistema registra un numero crescente di codici di correzione degli errori correggibili. In caso contrario, si verifica un panic di sistema. 
---
= Sostituisci un modulo DIMM nel tuo sistema FAS500f per risolvere gli errori di memoria
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
|===
. Afferra il modulo di controllo con entrambe le mani ed estrailo delicatamente dallo chassis, poi posizionalo su una superficie piana e stabile.
...
Per sostituire un modulo DIMM, è possibile utilizzare il seguente video o la seguente tabella:
...
+
I moduli DIMM devono essere inseriti saldamente nello zoccolo. Se un modulo DIMM non è inserito saldamente, reinseriscilo per riallinearlo con lo zoccolo.
...
+
.. Verifica che i bracci del meccanismo di bloccaggio siano bloccati nella posizione completamente estesa.
.. Con entrambe le mani, allinea e fai scorrere delicatamente il modulo di controllo nei bracci del meccanismo di bloccaggio fino a quando si ferma.
