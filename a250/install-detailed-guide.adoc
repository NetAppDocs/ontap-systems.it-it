...
---
= Installa il tuo sistema AFF A250 seguendo istruzioni dettagliate passo passo
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Installa il tuo sistema storage AFF A250 seguendo queste istruzioni dettagliate passo passo, dalla preparazione del sito all'avvio del cluster ONTAP.
...
.Fasi
. Usa il cavo di interconnessione del cluster da 25 GbE per collegare le porte di interconnessione del cluster e0c a e0c ed e0d a e0d.
+
...
NOTE: link:https://hwu.netapp.com["NetApp Hardware Universe"^] La priorità degli slot per le schede di rete host (Fibre Channel o 25 GbE) è lo slot 2. Tuttavia, se si dispone di entrambe le schede, la scheda Fibre Channel va nello slot 2 e la scheda 25 GbE va nello slot 1 (come mostrato nelle opzioni seguenti). Se si dispone di uno shelf esterno, la scheda di storage va nello slot 1, l'unico slot supportato per gli shelf.
...
--
.Opzione 2: Cavo per rete dati o host 25 GbE
--
Le porte 25 GbE sui controller sono collegate a switch di rete dati o host 25 GbE.
...
+
image:../media/drw_affa250_1shelf_cabling_a.png["Cablaggio del controller 1"]

. Collegare il controller B allo shelf.
+
image:../media/drw_affa250_1shelf_cabling_b.png["Cablaggio del controller 2"]
...
NOTE: Per informazioni su come stabilire la connettività continua di una porta della console del controller per la gestione remota, vedere l'articolo della Knowledge Base, https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/How_to_connect_an_ONTAP_controller_console_port_for_remote_management["Come collegare una porta della console del controller ONTAP per la gestione remota"^].
