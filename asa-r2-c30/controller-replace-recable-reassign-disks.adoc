...
summary: 'Restituisci il controllo delle risorse di storage al controller sostitutivo in modo che il sistema di storage ASA C30 possa riprendere il normale funzionamento. La procedura di restituzione varia in base al tipo di crittografia utilizzato dal sistema: nessuna crittografia, crittografia Onboard Key Manager (OKM) o crittografia External Key Manager (EKM).' 
---
