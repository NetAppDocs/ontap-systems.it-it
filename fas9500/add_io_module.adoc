...
---
= Aggiungi un modulo I/O a un sistema FAS9500
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Puoi aggiungere un modulo I/O al tuo sistema FAS9500 sostituendo una NIC o un adattatore di storage con uno nuovo in un sistema completamente popolato, oppure aggiungendo una nuova NIC o un nuovo adattatore di storage in uno slot chassis vuoto.
...
NOTE: Assicurati di installare dei tappi di chiusura negli slot I/O non utilizzati per prevenire possibili problemi termici.
...
. Se usi lo slot 3 o 7 per il networking, usa il comando  `storage port modify -node __<node name>__ -port __<port name>__ -mode network` per convertire lo slot per l'uso di networking.
