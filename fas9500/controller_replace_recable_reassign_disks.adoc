...
summary: Ricollegare i cavi del sistema e riassegnare i dischi. 
---
= Restituisci il controller nel tuo sistema FAS9500 per riprendere il normale funzionamento
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Restituisci il controllo delle risorse di storage al controller sostitutivo così il tuo sistema FAS9500 può riprendere il normale funzionamento.
...
.. Inserisci le informazioni relative al sistema di destinazione, quindi seleziona Raccogli dati.
.. Seleziona la scheda Cablaggio, quindi esamina l'output. Assicurati che vengano visualizzati tutti gli alloggiamenti per i dischi e che tutti i dischi siano presenti nell'output, correggendo eventuali problemi di cablaggio che trovi.
.. Verifica gli altri cablaggi selezionando la scheda appropriata e poi esaminando l'output di Config Advisor.





NOTE: L'ID di sistema e le informazioni di assegnazione del disco risiedono nel modulo NVRAM, che si trova in un modulo separato dal modulo controller e non viene influenzato dalla sostituzione del modulo controller.
...
Se il sistema storage è in una coppia HA, l'ID di sistema del nuovo modulo controller viene assegnato automaticamente ai dischi quando avviene il giveback al termine della procedura. Devi confermare la modifica dell'ID di sistema quando avvii il nodo di _sostituzione_ e poi verificare che la modifica sia corretta.

Questa procedura si applica solo ai sistemi che eseguono ONTAP in una coppia ha.

. Se il nodo _replacement_ è in modalità manutenzione (che mostra il `*>` Prompt), uscire dalla modalità di manutenzione e passare al prompt DEL CARICATORE: `halt`
. Dal prompt LOADER sul nodo _sostitutivo_, avvia il nodo, inserendo  `y` se ti viene richiesto di sovrascrivere l'ID di sistema a causa di una mancata corrispondenza dell'ID di sistema: `boot_ontap`
. Attendere il `Waiting for giveback...` Viene visualizzato sulla console del nodo _replacement_ e quindi, dal nodo integro, verificare che il nuovo ID di sistema del partner sia stato assegnato automaticamente: Visualizzazione del failover dello storage
+
...
.. Una volta completato il giveback, verificare che la coppia ha sia in buone condizioni e che sia possibile effettuare il takeover: Mostra il failover dello storage
+
