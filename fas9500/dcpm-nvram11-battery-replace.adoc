...
sidebar: sidebar 
keywords: FAS9500 
summary: Per eseguire lo swap a caldo di un modulo di alimentazione del controller di destage DCPM, che contiene la batteria NVRAM11, è necessario individuare il modulo DCPM guasto, rimuoverlo dallo chassis e installare il modulo DCPM sostitutivo. 
---
= Sostituisci il modulo DCPM e la batteria NVRAM11 nel tuo sistema FAS9500
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Devi avere a disposizione un modulo DCPM sostitutivo prima di rimuovere il modulo guasto dallo chassis. Devi sostituirlo entro cinque minuti dalla rimozione. Dopo la rimozione del modulo DCPM dallo chassis, non c'è alcuna protezione contro l'arresto per il modulo controller che possiede il modulo DCPM, se non il failover sull'altro modulo controller.
...
+
Il LED è fisso ambra se il modulo è difettoso.
...
+
image::../media/drw_a900_remove_NV_battery.svg[Sostituire la batteria NV (DCPM)]
