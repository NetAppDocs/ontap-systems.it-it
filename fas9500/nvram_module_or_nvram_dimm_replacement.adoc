...
---
= Sostituisci il modulo NVRAM o il DIMM NVRAM nel tuo sistema FAS9500
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Il modulo NVRAM nel tuo sistema FAS9500 è composto da NVRAM11 e DIMM. Puoi sostituire un modulo NVRAM guasto o un DIMM NVRAM per mantenere il normale funzionamento del sistema.

Per sostituire un modulo NVRAM guasto, devi rimuoverlo dallo chassis, spostare i DIMM sul modulo sostitutivo e installare il modulo NVRAM sostitutivo nello chassis.

Per sostituire un DIMM NVRAM, devi rimuovere il modulo NVRAM dallo chassis, sostituire il DIMM guasto nel modulo e poi reinstallare il modulo NVRAM.
...
Devi confermare la modifica dell'ID di sistema quando avvii il controller sostitutivo e poi verificare che la modifica sia corretta.
...
+
La configurazione MetroCluster impiega alcuni minuti dopo la sostituzione per tornare allo stato normale, momento in cui ogni controller mostra uno stato configurato, con DR Mirroring abilitato e modalità normale. L'output del comando  `metrocluster node show -fields node-systemid` visualizza il vecchio ID di sistema finché la configurazione MetroCluster non torna allo stato normale.
