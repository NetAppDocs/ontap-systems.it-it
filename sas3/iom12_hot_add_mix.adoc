...
* Lo stack di shelf IOM6, a cui si aggiunge a caldo uno shelf IOM12, deve essere cablato con cavi di rame SAS (per tutte le connessioni shelf-to-shelf e controller-to-stack).
+
I cavi ottici SAS non sono supportati in uno stack misto.

+

NOTE: Se lo shelf stack IOM6 è cablato con cavi ottici SAS, non è possibile aggiungere a caldo uno shelf IOM12. Contatta il tuo commerciale NetApp.

* Il sistema deve avere un numero inferiore al numero massimo di dischi supportati, almeno del numero di dischi che è possibile installare negli shelf IOM12 che si stanno aggiungendo a caldo.
+
...
+
Gli shelf IOM12 utilizzano connettori mini-SAS HD. Gli shelf IOM6 utilizzano connettori QSFP.
...
* Se le porte SAS bridge sono disponibili nella configurazione corrente, aggiungere gli shelf IOM12 come stack separato.
+
...
* Quando si aggiungono a caldo shelf con moduli IOM12 a uno stack di shelf con moduli IOM6, le prestazioni dell'intero stack funzionano a 6 Gbps (funziona alla velocità comune più bassa).
+
Se gli shelf che si stanno aggiungendo a caldo sono shelf che sono stati aggiornati dai moduli IOM3 o IOM6 ai moduli IOM12, lo stack funziona a 12 Gbps; tuttavia, le funzionalità del disco e del backplane dello shelf possono limitare le prestazioni dei dischi a 3 Gbps o 6 Gbps.
...
+
** Per i sistemi in cui IBAP è in esecuzione, IBACP viene attivato automaticamente sugli shelf IOM12 con aggiunta a caldo.
** Per i sistemi in cui è abilitato l'ACP out-of-band, le funzionalità ACP non sono disponibili sugli shelf IOM12.
+
...
Se si assegna manualmente la proprietà del disco per gli shelf IOM12 che si stanno aggiungendo a caldo, è necessario disattivare l'assegnazione automatica del disco, se attivata.
...
+
Un ID shelf valido va da 00 a 99. Si consiglia di impostare gli ID dello shelf in modo che gli shelf IOM6 utilizzino numeri inferiori (1 - 9) e gli shelf IOM12 utilizzino numeri superiori (10 e superiori).
...
Il modo in cui si collega uno shelf IOM12/IOM12B a uno stack di shelf IOM6 dipende dal fatto che lo shelf IOM12 sia lo shelf IOM12/IOM12B iniziale, il che significa che non esistono altri shelf IOM12 nello stack, Oppure se si tratta di uno shelf IOM12/IOM12B aggiuntivo per uno stack misto esistente, il che significa che uno o più shelf IOM12/IOM12B sono già presenti nello stack. Dipende anche dal fatto che lo stack disponga di connettività ha multipath, multipath, ha a percorso singolo o a percorso singolo.
...
** In una coppia ha con storage IOM12E integrato, una singola transizione di velocità all'interno di uno stack avente due shelf IOM12 e due shelf IOM6 è rappresentata come:
+
 IOM12E 0b <-> IOM12/IOM12B <-> IOM12/IOM12B <---> IOM6 <-> IOM6 <-> IOM12E 0a
+
La porta storage integrata 0b è la porta dello storage interno (espansore) e, poiché si collega allo shelf IOM12/IOM12B aggiunto a caldo (l'ultimo shelf nello stack), il gruppo di shelf IOM12/IOM12B viene mantenuto insieme e viene mantenuta una singola transizione attraverso lo stack e lo storage IOM12E integrato.
...
+
Questa illustrazione è specifica di uno stack con connettività ha multipath; tuttavia, il concetto di cablaggio può essere applicato a stack con configurazioni multipercorso, ha a percorso singolo, a percorso singolo e MetroCluster stretch.

+
image::../media/drw_sas2_sas3_mixed_stack.png[stack misto drw sas2 sas3]

* Per il cablaggio degli shelf IOM12/IOM12B su uno shelf stack IOM6 in una configurazione MetroCluster con collegamento a ponte, fare riferimento alla seguente illustrazione. image:../media/hot_adding_iom12_shelves_to_iom6_stack_in_bridge_attached_config.png[""]


.Fasi
. Identificare fisicamente l'ultimo shelf logico nello stack.
+
A seconda del modello di piattaforma e della connettività dello stack (ha multipath, multipath, ha single-path o single-path), l'ultimo shelf logico è lo shelf con connessioni controller-to-stack dalle porte SAS del controller B e D, In alternativa, lo shelf non dispone di connessioni a nessun controller (perché la connettività controller-to-stack è al top logico dello stack, attraverso le porte SAS A e C del controller).
...
| Se la connettività dello stack IOM6 è... | Quindi... 
 ...
 a| 
Ha multipath o multipath o ha a percorso singolo con connettività del controller all'ultimo shelf logico (incluse le configurazioni Stretch MetroCluster)
 a| 
...
.. Ristabilire la connessione controller-to-stack cablando la stessa porta sul controller o sul bridge (nella fase a) alla nuova porta IOM A 3 dell'ultimo shelf IOM12.
+
...
.. Collegare il collegamento shelf-to-shelf tra l'ultima porta IOM A circolare dello shelf IOM6 (dalla fase a) alla nuova porta IOM A dello shelf IOM12 1.
+
Utilizzare un cavo SAS QSFP-to-Mini-SAS HD in rame.
...
.. Ristabilire la connessione bridge-to-stack inferiore cablando la stessa porta sul bridge (nella fase a) alla nuova porta IOM A 3 dell'ultimo shelf IOM12.
+
Utilizzare un cavo SAS HD da QSFP a Mini SAS in rame o un cavo Mini SAS HD-Mini SAS HD-SAS, a seconda del tipo di porta sul controller.
...
| Se la connettività dello stack misto è... | Quindi... 
 ...
 a| 
Ha multipath o multipath o ha a percorso singolo con connettività controller all'ultimo shelf logico o connettività bridge-attached in una configurazione MetroCluster
 a| 
