...
summary: Definisci le coppie di porte SAS del controller che puoi utilizzare per collegare i controller agli stack di shelf di dischi per la connettività a quattro percorsi in una coppia HA o in una configurazione a singolo controller. 
---
= Schema di cablaggio per la connettività quad-path di DS212C, DS224C o DS460C
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Puoi usare un foglio di lavoro per il cablaggio per definire le coppie di porte SAS del controller che puoi usare per collegare i controller a stack di shelf di dischi con moduli IOM12, IOM12B o IOM12C, per ottenere una connettività a quattro percorsi in una coppia HA o in una configurazione a singolo controller. Puoi anche usare il foglio di lavoro compilato per guidarti nel cablaggio delle connessioni a quattro percorsi per la tua configurazione.
...
+
Per gli esempi del foglio di lavoro viene utilizzata una configurazione con due HBA SAS a quattro porte e due stack di shelf di dischi con moduli IOM12, IOM12B o IOM12C.
