...
summary: Utilizza questo esempio come guida per leggere e applicare un foglio di lavoro completato per cablare i collegamenti controller-to-stack per shelf di dischi per la connettività multipath. 
---
= Leggi una scheda di cablaggio per le connessioni multipath controller-stack DS212C, DS224C o DS460C
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Puoi usare questo esempio come guida per leggere e applicare un foglio di lavoro compilato per cablare i collegamenti controller-to-stack per shelf di dischi con moduli IOM12, IOM12B o IOM12C per la connettività multipath.
...
+
La configurazione utilizzata in questo esempio è una configurazione HA multipath con due HBA SAS a quattro porte (otto porte SAS) su ciascun controller e due gruppi di unità disco con moduli IOM12, IOM12B o IOM12C. Le coppie di porte vengono cablate saltando ogni altra coppia di porte nel foglio di lavoro.
