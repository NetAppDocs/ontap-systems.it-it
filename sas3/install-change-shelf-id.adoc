...
summary: 'Cambia l"ID di uno shelf in un sistema quando ONTAP non è ancora in esecuzione, quando aggiungi uno shelf a caldo prima che venga collegato al sistema, oppure quando ONTAP è attivo e tutte le unità nello shelf non sono di proprietà, sono spare o fanno parte di aggregati offline.' 
---
= Cambia l'ID dello shelf DS212C, DS224C o DS460C
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Puoi cambiare l'ID di uno shelf DS212C, DS224C o DS460C quando ONTAP non è ancora in esecuzione o quando aggiungi uno shelf a caldo prima che venga collegato al sistema. Puoi anche cambiare l'ID di uno shelf quando ONTAP è attivo e in esecuzione (i moduli controller sono disponibili per servire i dati) e tutti i dischi nello shelf sono non assegnati, spare o parte di aggregati offline.
...
. Ripetere i passaggi precedenti per ogni ripiano disco aggiuntivo.
