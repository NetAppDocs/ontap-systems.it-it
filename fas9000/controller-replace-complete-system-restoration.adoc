...
summary: Per completare la procedura di sostituzione e ripristinare il pieno funzionamento del sistema, è necessario ricablare lo storage, ripristinare la configurazione di NetApp Storage Encryption, se necessario, e installare le licenze per il nuovo controller. 
---
= Completa la sostituzione del controller per il tuo sistema FAS9000
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Per completare la sostituzione del controller per il tuo sistema FAS9000, ricabla lo storage, ripristina la configurazione di NetApp Storage Encryption se necessario e installa le licenze per il nuovo controller.
...
== Passaggio 3 (MetroCluster solo): Passa di nuovo gli aggregati in una configurazione MetroCluster a due nodi
