...
summary: Prima di spegnere il controller danneggiato e verificare lo stato delle chiavi di crittografia integrate, devi controllare lo stato del controller danneggiato, disabilitare il giveback automatico e verificare quale versione di ONTAP è in esecuzione sul sistema. Il sistema FAS9000 supporta solo procedure manuali di ripristino del boot media. 
---
= Verifica il supporto della crittografia sul supporto di avvio in un sistema FAS9000
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Per garantire la sicurezza dei dati sul tuo sistema storage, verifica il supporto e lo stato della chiave di crittografia sul supporto di boot. Controlla se la tua versione di ONTAP supporta NetApp Volume Encryption (NVE) e, prima di spegnere il controller, verifica che il key manager sia attivo. Il sistema FAS9000 supporta solo procedure manuali di ripristino del supporto di boot.
