...
summary: 'Una volta verificate le variabili d"ambiente, è necessario completare i passaggi specifici per i sistemi che hanno Onboard Key Manager, NetApp Storage Encryption o NetApp Volume Encryption abilitati.' 
---
= Ripristina la crittografia sul supporto di avvio nel tuo sistema ASA A800 dopo la procedura di ripristino manuale dell'avvio
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ripristinare la crittografia sul supporto di avvio sostitutivo nel sistema ASA A800 per garantire una protezione continua dei dati. Il processo di sostituzione prevede la verifica della disponibilità della chiave, la riapplicazione delle impostazioni di crittografia e la conferma dell'accesso sicuro ai dati.
