...
summary: Sostituisci il controller nel tuo sistema FAS2600 rimuovendo il controller danneggiato, trasferendo i componenti FRU sul modulo controller di ricambio, installandolo nello chassis e avviando il sistema in modalità Maintenance. 
---
= Sostituisci il controller nel tuo sistema FAS2600
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sostituisci il controller nel tuo sistema FAS2600 rimuovendo il controller danneggiato, spostando i componenti FRU sul modulo controller di ricambio, installando il modulo controller di ricambio nello chassis e poi avviando il sistema in modalità manutenzione.

link:https://youtu.be/2_uWRt_iJqk["Video sulla sostituzione del controller AFF FAS2600"]
...
Per spostare il modulo di caching, indicato come scheda M.2 PCIe sull'etichetta del controller, dal vecchio modulo controller al modulo controller sostitutivo, segui questa sequenza di passaggi.

Devi avere pronto il nuovo modulo controller così puoi spostare il modulo di caching direttamente dal vecchio modulo controller allo slot corrispondente nel nuovo. Tutti gli altri componenti del sistema storage devono funzionare correttamente. Se non è così, contatta il supporto tecnico.
...
Per le coppie HA con due moduli controller nello stesso chassis, l'ordine in cui installi i moduli controller è particolarmente importante perché un modulo controller tenta di riavviarsi non appena è completamente inserito nello chassis.
