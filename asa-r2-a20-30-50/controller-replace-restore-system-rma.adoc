...
summary: Per completare la sostituzione del controller per il sistema storage ASA A20, ASA A30 o ASA A50, ripristina innanzitutto la configurazione di NetApp Storage Encryption e installa le licenze necessarie sul nuovo controller. Successivamente, conferma che le interfacce logiche stiano riportando alle rispettive porte home ed esegui un controllo dello stato di salute del cluster. Infine, registra il numero di serie del nuovo controller e poi restituisci il componente guasto a NetApp. 
---
= Completa la sostituzione del controller per il tuo sistema ASA A20, ASA A30 o ASA A50
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Per completare la sostituzione del controller per il sistema di storage ASA A20, ASA A30 o ASA A50, ripristinare innanzitutto la configurazione di NetApp Storage Encryption (se necessario) e installare le licenze richieste sul nuovo controller.  Successivamente, verificare che le interfacce logiche (LIF) segnalino le rispettive porte home ed eseguire un controllo dello stato del cluster.  Infine, registra il numero di serie del nuovo controller e restituisci la parte guasta a NetApp.
...
Prima di rimettere in servizio il nodo _sostitutivo_, è necessario verificare che i LIF siano sulle rispettive porte principali, registrare il numero di serie del nodo _sostitutivo_ se AutoSupport è abilitato, controllare lo stato di salute del cluster, chiudere eventuali finestre di manutenzione e reimpostare il ripristino automatico.
