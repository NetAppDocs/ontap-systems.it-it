...
summary: 'Inizia a sostituire il boot media nel tuo sistema storage ASA A250 esaminando i requisiti di sostituzione, spegnendo il controller, sostituendo il boot media, ripristinando l"immagine sul boot media e verificando la funzionalità del sistema.' 
---
= Flusso di lavoro per la sostituzione del supporto di avvio per un sistema ASA A250 utilizzando il processo di ripristino automatico dell'avvio
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Il ripristino automatico dell'immagine di avvio prevede che il sistema identifichi e selezioni automaticamente l'opzione appropriata del menu di boot. Utilizza l'immagine di avvio sul nodo partner per reinstallare ONTAP sul supporto di avvio sostitutivo nel sistema storage ASA A250.
