...
. Se il nodo non funzionante fa parte di una configurazione ha, disattivare il giveback automatico dal nodo integro: `storage failover modify -node local -auto-giveback false` oppure `storage failover modify -node local -auto-giveback-after-panic false`
