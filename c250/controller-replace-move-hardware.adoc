...
---
= Sostituisci il controller nel sistema storage AFF C250
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sostituisci il controller nel tuo sistema AFF C250 quando un guasto hardware lo richiede. La procedura di sostituzione prevede la rimozione del controller danneggiato, lo spostamento dei componenti sul controller sostitutivo, l'installazione del controller sostitutivo e il suo riavvio.
...
Dopo aver sostituito il controller AFF C250 danneggiato, è necessario link:controller-replace-system-config-restore-and-verify.html["ripristina e verifica la configurazione del sistema"].
