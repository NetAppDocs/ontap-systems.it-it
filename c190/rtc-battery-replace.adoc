...
summary: Sostituire la batteria real-time clock RTC nel modulo controller affinché i servizi e le applicazioni del sistema che dipendono da una sincronizzazione oraria precisa continuino a funzionare. 
---
= Sostituisci la batteria dell'orologio in tempo reale nel tuo sistema AFF C190
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Per sostituire la batteria RTC, è necessario individuarla all'interno del modulo controller, quindi seguire la sequenza specifica di passaggi.

. Individuare la batteria RTC.
+
image::../media/drw_2600_rtc_battery.gif[Sostituire la batteria RTC]
...
. Completare la reinstallazione del modulo controller. Il modulo controller inizia ad avviarsi non appena viene inserito completamente nello chassis.
+
