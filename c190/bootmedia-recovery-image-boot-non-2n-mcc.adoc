...
summary: 'Avviare l"immagine di ripristino ONTAP dall"unità USB su un sistema AFF C190 per ripristinare il supporto di avvio. Il sistema AFF C190 supporta solo procedure di ripristino manuale tramite supporti di avvio. Il ripristino automatico del supporto di avvio non è supportato.' 
---
= Avvia l'immagine di ripristino sul tuo sistema AFF C190
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Avviare l'immagine di ripristino ONTAP dall'unità USB su un sistema AFF C190 per ripristinare il supporto di avvio. Questa procedura include l'avvio dall'unità flash USB, il ripristino del file system, la verifica delle variabili ambientali e il ripristino del normale funzionamento del controller dopo la sostituzione del supporto di avvio.
...
.Fasi
. Dal prompt DEL CARICATORE, avviare l'immagine di ripristino dall'unità flash USB:
+
`*boot_recovery*`
