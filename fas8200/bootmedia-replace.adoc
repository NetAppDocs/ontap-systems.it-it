...
summary: Sostituire il supporto di avvio non riuscito su un modulo controller FAS8200 . Il sistema FAS8200 supporta solo procedure di ripristino manuale tramite supporti di avvio. Il ripristino automatico del supporto di avvio non è supportato. 
---
= Sostituisci il supporto di avvio in un sistema FAS8200
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Il file tar.gz deve essere posizionato su una partizione formattata in FAT32 di almeno 4GB. Sebbene le partizioni FAT32 possano essere grandi fino a 2TB, gli strumenti integrati di Windows (ad esempio, DiskPart) non possono formattare partizioni FAT32 più grandi di 32GB.
...
+
Il sistema implementa le modifiche quando si avvia.
