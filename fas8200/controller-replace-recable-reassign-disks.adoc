...
summary: Dopo aver sostituito il controller, ricollega i cavi del sistema e verifica che la proprietà del disco sia stata riassegnata, così il tuo sistema FAS8200 può riprendere le normali operazioni. 
---
= Restituisci il controller nel tuo sistema FAS8200 per riprendere il normale funzionamento
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Dopo aver sostituito il controller, ricollega i cavi del sistema e verifica che la proprietà del disco sia stata riassegnata, così il tuo sistema FAS8200 può riprendere le normali operazioni.
...
... Attendere `savecore` comando da completare prima di emettere il giveback.
+
È possibile immettere il seguente comando per monitorare l'avanzamento di `savecore` comando: `system node run -node _local-node-name_ partner savecore -s`
