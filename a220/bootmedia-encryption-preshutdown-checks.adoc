...
summary: Prima di spegnere il controller non funzionante e verificare lo stato delle chiavi di crittografia integrate, è necessario controllare lo stato del controller non funzionante, disabilitare il giveback automatico e verificare quale versione di ONTAP è in esecuzione sul sistema. Il sistema AFF A220 supporta solo procedure di ripristino manuale tramite supporti di avvio. 
---
= Verifica il supporto della crittografia sul supporto di avvio nel tuo sistema AFF A220
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Per garantire la sicurezza dei dati sul sistema di archiviazione, è necessario verificare il supporto e lo stato della chiave di crittografia sul supporto di avvio. Verificare se la versione ONTAP supporta NetApp Volume Encryption (NVE) e, prima di arrestare il controller, verificare che il gestore delle chiavi sia attivo. Il sistema AFF A220 supporta solo procedure di ripristino manuale tramite supporti di avvio. Il ripristino automatico del supporto di avvio non è supportato.
