...
---
= Verifica il supporto della crittografia sul supporto di avvio in un sistema FAS8300 o FAS8700 usando la procedura di ripristino manuale dell'avvio
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Per garantire la sicurezza dei dati sul tuo sistema storage, devi verificare il supporto e lo stato della chiave di crittografia sul supporto di avvio. Controlla se la versione di ONTAP supporta NetApp Volume Encryption (NVE). Prima di spegnere il controller, controlla se il key manager è attivo.
