...
summary: 'È necessario sostituire un DIMM nel controller quando il sistema storage riscontra errori come un numero eccessivo di codici di correzione degli errori correggibili o errori non correggibili, in genere causati dal guasto di un singolo DIMM che impedisce al sistema storage l"avvio di ONTAP.' 
---
= Sostituisci un modulo DIMM nel tuo sistema FAS8300 o FAS8700 per risolvere gli errori di memoria
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Devi sostituire un DIMM nel controller quando il tuo sistema storage riscontra errori come un numero eccessivo di errori CECC (Correctable Error Correction Code) basati sugli avvisi di Health Monitor o errori ECC non correggibili. Questi errori sono generalmente causati dal guasto di un singolo DIMM che impedisce al sistema storage di avviare ONTAP.

Tutti gli altri componenti del sistema devono funzionare correttamente. Se un componente non funziona correttamente, devi contattare il supporto tecnico.
...
Il numero e la posizione dei DIMM nel tuo sistema dipendono dal modello del tuo sistema. Consulta la mappa FRU sul condotto dell'aria per ulteriori informazioni.

* Se si dispone di un sistema FAS8300, i DIMM di sistema si trovano negli zoccoli 2, 4, 13 e 15.
* Se si dispone di un sistema FAS8700, i DIMM di sistema si trovano negli slot 2, 4, 7, 9, 13, 15, 18 e 20.
* Il modulo NVDIMM si trova nello slot 11.
...
+
Il modulo DIMM si inserisce saldamente nello slot, ma dovrebbe entrare facilmente. Se non entra, riallinea il modulo DIMM con lo slot e reinseriscilo.
...
Per installare il modulo controller nel telaio, è possibile utilizzare l'animazione, il disegno o le fasi scritte riportate di seguito.
...
NOTE: Colleghi il resto dei cavi al modulo di controllo più avanti in questa procedura.

. Completare l'installazione del modulo controller:
+
.. Spingi con decisione il modulo di controllo nello chassis utilizzando i fermi di bloccaggio finché questi non iniziano a sollevarsi.
+
