...
.Fasi
. Se il controller da sottoporre a manutenzione non viene visualizzato al prompt DEL CARICATORE, riavviare il controller: `system node halt -node _node_name_`
+
...
** Se il test non ha rilevato errori, selezionare `Reboot` dal menu per riavviare il sistema.
