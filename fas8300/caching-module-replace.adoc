...
summary: È necessario sostituire il modulo di caching nel modulo controller quando il sistema registra un singolo messaggio AutoSupport che indica che il modulo è andato offline. 
---
= Sostituisci il modulo di caching nel tuo sistema FAS8300 o FAS8700
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Il modulo controller VER2 ha solo un socket per modulo di caching nel FAS8300. FAS8700 non ha un modulo controller VER2. La funzionalità del modulo di caching non è influenzata dalla rimozione del socket.
...
* Sebbene il contenuto del modulo di caching sia crittografato, è best practice cancellarne il contenuto prima di sostituirlo. Per ulteriori informazioni, consulta la  https://mysupport.netapp.com/info/web/ECMP1132988.html["Dichiarazione di volatilità"] per il tuo sistema sul sito di supporto NetApp.
+

NOTE: Per visualizzare la _Dichiarazione di volatilità_ del sistema, è necessario accedere al sito di supporto NetApp.

* Tutti gli altri componenti del sistema storage devono funzionare correttamente. Se non è così, contatta il supporto tecnico.
...
. Individua il modulo di caching guasto utilizzando la mappa FRU sul modulo controller e rimuovilo:
+
A seconda della configurazione, nel modulo controller potrebbero esserci zero, uno o due moduli di caching. Usa la mappa FRU all'interno del modulo controller per individuare il modulo di caching.
...
== Passaggio 7: Completa il processo di sostituzione
