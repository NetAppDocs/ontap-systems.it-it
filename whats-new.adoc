...
.sistema storage AFX 2K
Il nuovo NetApp AFX 2K sistema storage estende la soluzione disaggregata progettata per soddisfare i rigorosi requisiti dei carichi di lavoro NAS e S3 dalle performance elevate, incluse le applicazioni di AI/ML. I sistemi AFX 2K offrono funzionalità specializzate per i clienti che necessitano di storage file e a oggetti scalabile e ad alto throughput.

link:https://docs.netapp.com/us-en/ontap-afx/get-started/ontap-afx-storage.html["Scopri di più sul sistema AFX"] .

.Nuovo supporto per switch Cisco 9808 per AFX 2K
I nuovi switch Cisco 9808 si connettono ai nodi controller AFX 2K con le shelf di storage. Questo nuovo switch, insieme ai Cisco 9364D-GX2A e Cisco 9332D-GX2B, ti permette di creare cluster ONTAP con più di due nodi. Vengono utilizzati protocolli avanzati per ottimizzare le performance. Il design si basa sul tagging VLAN con percorsi di rete multipli, oltre a configurazioni di tech-refresh, per garantire operatività continua e facilità di upgrade.
...
=== Procedure di swap a caldo del modulo I/O

Le procedure di swap a caldo sono ora disponibili per moduli I/O selezionati su più piattaforme hardware ONTAP e versioni di ONTAP.

*Supportato con ONTAP 9.18.1 GA o versioni successive:*

È possibile eseguire lo swap a caldo di un modulo I/O Ethernet guasto in qualsiasi slot, a condizione che le sue porte siano utilizzate per connessioni cluster, HA o client. Questa procedura è supportata su sistemi con qualsiasi numero di nodi.


NOTE: Non è possibile eseguire lo swap a caldo dei moduli I/O Ethernet che hanno porte assegnate allo storage o alle connessioni MetroCluster.
...
*Supportato con ONTAP 9.17.1 o 9.18.1RC:*

Per evitare un'interruzione in un cluster a due nodi, è possibile effettuare lo swap a caldo di un modulo cluster e HA I/O nello slot 4 solo se il controller danneggiato (quello con il modulo guasto) ha già effettuato il takeover del partner funzionante. Questo è l'unico metodo di ripristino supportato che non richiede downtime.
...
=== Fine disponibilità AFF C800

Il sistema AFF C800 è stato spostato nell'elenco di prodotti non più disponibili e non è più disponibile per nuovi ordini.

link:endofavail/index.html["Visualizza l'hardware a fine disponibilità"].
