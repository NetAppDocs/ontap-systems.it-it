...
---
= Sostituisci il supporto del modulo Flash Cache o il modulo di caching nel tuo sistema FAS70 o FAS90
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Prima di iniziare
* Verifica che il tuo sistema storage abbia il sistema operativo appropriato per il supporto del modulo Flash Cache di ricambio.
* Verifica che tutti gli altri componenti funzionino correttamente. Se non lo fanno, contatta il supporto tecnico.
...
.Prima di iniziare
* Verifica che il modulo di cache sostitutivo abbia la stessa capacità di quello guasto, che sia dello stesso fornitore o di un fornitore diverso supportato.
* Verifica che tutti gli altri componenti funzionino correttamente. Se non lo fanno, contatta il supporto tecnico.
...
.. Rimuovi il modulo di cache dall'alloggiamento inserendo il dito nell'apertura della leva a camma ed estraendo il modulo dal supporto del modulo Flash Cache.
+
...
+
Assicurati che l'output del comando riporti lo stato come  `powered-on` e pronto per l'operazione.
