...
[role="lead"]
Ripristina il controllo delle risorse di storage sul controller sostitutivo, in modo che il sistema FAS70 o FAS90 possa riprendere il normale funzionamento. La procedura di give back varia in base al tipo di crittografia utilizzato dal sistema: Nessuna crittografia, crittografia OKM (Onboard Key Manager) o EKM (External Key Manager).
