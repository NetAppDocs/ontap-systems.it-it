...
summary: È possibile eseguire lo swap a caldo di un modulo I/O Ethernet nel sistema storage AFF C30 o AFF C60 se un modulo si guasta e il sistema storage soddisfa tutti i requisiti della versione di ONTAP. 
---
= Sostituisci a caldo un modulo I/O nel tuo sistema storage AFF C30 o AFF C60
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
È possibile eseguire lo swap a caldo di un modulo I/O Ethernet nel sistema storage AFF C30 o AFF C60 se un modulo si guasta e il sistema storage soddisfa tutti i requisiti della versione di ONTAP.

Per eseguire lo swap a caldo di un modulo I/O, verifica che il tuo sistema storage soddisfi i requisiti della versione di ONTAP, prepara il sistema storage e il modulo I/O, sostituisci a caldo il modulo guasto, metti online il modulo sostitutivo, ripristina il sistema storage al normale funzionamento e restituisci il modulo guasto a NetApp.
...
+
** Il controller con il modulo I/O danneggiato è il controller su cui stai effettuando la sostituzione a caldo del modulo I/O.
** Il controller funzionante è la coppia HA del controller con il modulo I/O danneggiato.
...
[[step-1-verify-the-storage-system-meets-the-procedure-requirements]]
== Fase 1: Verifica che il sistema storage soddisfi i requisiti della procedura
