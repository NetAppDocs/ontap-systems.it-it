...
---
= Aggiungi un modulo I/O al tuo sistema storage AFF C30 o AFF C60 per migliorare la connettività di rete
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Aggiungete un modulo I/O al vostro sistema storage AFF C30 o AFF C60 per migliorare la connettività di rete ed espandere la capacità del sistema di gestire il traffico dati. Potete utilizzare questa procedura per aggiungere un modulo I/O a un controller operativo come aggiornamento pianificato oppure a un controller danneggiato durante la manutenzione.
...
È possibile eseguire questa procedura su uno qualsiasi dei controller della coppia HA, indipendentemente dal fatto che il controller sia attualmente operativo o non funzionante. Poiché non è possibile aggiungere un modulo I/O a un controller in esecuzione, il controller di destinazione deve essere arrestato prima dell'installazione del modulo. Il partner HA assume il controllo delle operazioni di storage durante la procedura, in modo che i dati rimangano disponibili per i client.

* Se il controller di destinazione è *operativo* (aggiunta pianificata), seguire i passaggi di arresto come descritto, trattando il controller di destinazione come il controller "impaired" in quel contenuto.
* Se il controller di destinazione è *danneggiato*, seguire i passaggi di spegnimento come descritto.
...
Assumi il controllo e spegni il modulo controller di destinazione in modo che il suo partner HA continui a fornire dati mentre aggiungi il modulo I/O. I passaggi collegati si riferiscono al controller che viene spento come controller "danneggiato"; gli stessi passaggi si applicano sia che tu stia assumendo il controllo di un controller operativo per un'aggiunta pianificata sia che tu stia spegnendo un controller già danneggiato.
