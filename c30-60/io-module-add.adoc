...
summary: Aggiungi un modulo i/o al sistema AFF C30 o AFF C60 per migliorare la connettività di rete e ampliare la capacità del sistema di gestire il traffico dei dati. 
---
...
[role="lead"]
Aggiungi un modulo i/o al sistema AFF C30 o AFF C60 per migliorare la connettività di rete e ampliare la capacità del sistema di gestire il traffico dei dati.
...
. Restituire il controller danneggiato dal controller in buone condizioni: `storage failover giveback -ofnode _impaired_node_name_`
...
. Ripristino del giveback automatico dalla console del controller integro: `storage failover modify -node local -auto-giveback _true_`
. Se AutoSupport è abilitato, ripristinare (riattivare) la creazione automatica dei casi: `system node autosupport invoke -node * -type all -message MAINT=END`
. Se è stato installato un modulo NIC, specificare la modalità di utilizzo per ciascuna porta come _network_: `storage port modify -node _node_name_ -port _port_name_ -mode _network_`
