...
summary: Aggiungi un modulo I/O al tuo sistema storage AFF C30 o AFF C60 per migliorare la connettività di rete ed espandere la capacità del sistema di gestire il traffico dati. 
---
= Aggiungi un modulo I/O a un sistema AFF C30 o AFF C60
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Aggiungi un modulo I/O al tuo sistema storage AFF C30 o AFF C60 per migliorare la connettività di rete ed espandere la capacità del sistema di gestire il traffico dati. Puoi usare questa procedura per aggiungere un modulo I/O sia a un controller operativo come upgrade pianificato, sia a un controller con un modulo I/O guasto durante la manutenzione.

È possibile aggiungere un modulo i/o ai sistemi di storage AFF C30 e AFF C60 quando sono disponibili slot o quando tutti gli slot sono completamente popolati.

.A proposito di questa attività
Puoi eseguire questa procedura su uno qualsiasi dei controller della coppia HA, indipendentemente dal fatto che il modulo I/O del controller sia attualmente operativo o guasto. Poiché non è possibile aggiungere un modulo I/O a un controller in esecuzione, il controller di destinazione deve essere spento prima che il modulo venga installato. Il partner HA si occupa delle operazioni di storage durante la procedura, così i dati restano disponibili per i client.

* Se il controller di destinazione è *operativo* (aggiunta pianificata), segui i passaggi di arresto come descritto, trattando il controller di destinazione come se avesse un modulo I/O danneggiato in quella procedura.
* Se il controller di destinazione presenta un modulo I/O danneggiato, segui i passaggi di spegnimento come descritto.
...
== Passaggio 1: Spegnere il modulo controller di destinazione

Assumi il controllo e spegni il controller di destinazione in modo che il suo partner HA continui a fornire dati mentre aggiungi il modulo I/O. I passaggi collegati si riferiscono allo spegnimento del controller che ospita il modulo I/O difettoso. Questi passaggi si applicano sia che tu stia assumendo il controllo di un controller per un'aggiunta pianificata, sia che tu stia spegnendo un controller che ha già un modulo I/O difettoso.
...
.Prima di iniziare
* Controlla  https://hwu.netapp.com["NetApp Hardware Universe"^] per assicurarti che il nuovo modulo I/O sia compatibile con il tuo sistema storage e con la versione di ONTAP che stai utilizzando.
