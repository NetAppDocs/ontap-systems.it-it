...
---
= Verifica il supporto di crittografia sul supporto di avvio del tuo sistema storage AFF C30 o AFF C60
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Per garantire la sicurezza dei dati sul sistema storage AFF C30 o AFF C60, è necessario verificare il supporto e lo stato della chiave di crittografia sul supporto di avvio. Controlla se la versione di ONTAP supporta NetApp Volume Encryption (NVE). Prima di spegnere il controller, controlla se il key manager è attivo.
...
Prima di spegnere il controller con il supporto di avvio danneggiato, verifica la configurazione del key manager ed esegui il backup delle informazioni necessarie.
