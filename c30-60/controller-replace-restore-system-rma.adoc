...
summary: Per completare la sostituzione del controller per il sistema di storage AFF C30 o AFF C60, ripristinare prima la configurazione di NetApp Storage Encryption, se necessario, e installare le licenze richieste sul nuovo controller.  Successivamente, verificare che le interfacce logiche LIF segnalino alle rispettive porte home ed eseguire un controllo dello stato del cluster.  Infine, registra il numero di serie del nuovo controller e poi restituisci la parte guasta a NetApp 
---
= Completa la sostituzione del controller per il tuo sistema storage AFF C30 o AFF C60
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
