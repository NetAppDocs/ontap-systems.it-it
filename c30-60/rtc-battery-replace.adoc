...
summary: 'Sostituisci la batteria dell"orologio in real-time, comunemente nota come batteria a bottone, nel tuo sistema storage AFF C30 o AFF C60 per garantire che i servizi e le applicazioni che si basano su una sincronizzazione oraria precisa rimangano operativi.' 
---
= Sostituisci la batteria dell'orologio in real-time nel tuo sistema storage AFF C30 o AFF C60
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sostituisci la batteria dell'orologio in tempo reale (RTC), comunemente nota come batteria a bottone, nel tuo sistema di archiviazione AFF C30 o AFF C60 per garantire che i servizi e le applicazioni che si basano su una sincronizzazione oraria accurata rimangano operativi.

.Prima di iniziare
Tutti gli altri componenti del sistema storage devono funzionare correttamente. In caso contrario, contatta https://mysupport.netapp.com/site/global/dashboard["Supporto NetApp"] prima di continuare con questa procedura.
...
+
Puoi spegnerli inserendo il comando  `system location-led _off_`. Se non sei sicuro se i LED siano accesi o spenti, controlla il loro stato inserendo il comando  `system location-led show`.
...
|===
. Apri il coperchio del controller ruotando la vite a testa zigrinata in senso antiorario.
...
NOTE: Dopo aver sostituito la batteria RTC, inserito il controller e acceso il dispositivo, durante il primo ripristino del BIOS potresti vedere i seguenti messaggi di errore:
