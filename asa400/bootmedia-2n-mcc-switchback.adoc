...
summary: 'Dopo aver completato la sostituzione dell"FRU in una configurazione MetroCluster a due nodi, è possibile eseguire l"operazione di switchback MetroCluster. Questa operazione riporta la configurazione al suo stato operativo normale, con le macchine virtuali di storage di origine sincrona (SVM) sul sito precedentemente interessato ora attive e che forniscono dati dai pool di dischi locali.' 
---
= Commuta nuovamente gli aggregati in una configurazione a due nodi MetroCluster nel tuo sistema ASA A400
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
