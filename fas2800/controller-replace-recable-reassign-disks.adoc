...
summary: 'Per completare la sostituzione del controller e ripristinare il sistema FAS2820 al pieno funzionamento, devi ricablare lo storage, confermare la riassegnazione dei dischi, ripristinare la configurazione di NetApp Storage Encryption (se necessario) e installare le licenze per il nuovo controller.' 
---
= Restituisci il controller nel tuo sistema FAS2820 per riprendere il normale funzionamento
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Per completare la procedura di sostituzione e ripristinare il tuo sistema FAS2820 al pieno funzionamento, devi ricablare lo storage, confermare la riassegnazione dei dischi, ripristinare la configurazione di NetApp Storage Encryption (se necessario) e installare le licenze per il nuovo controller.
...
.. Attendere il `savecore` comando da completare prima di emettere il giveback.
+
...
IMPORTANT: Se il giveback viene veto, risolvere il problema del veto. Se il veto non è critico da risolvere, è possibile ignorare il veto.
