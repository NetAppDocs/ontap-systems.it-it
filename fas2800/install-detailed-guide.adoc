...
sidebar: sidebar 
keywords: fas2820, detailed, guide, prepare, install, hardware, cable, controllers, node, switchless, cluster, network, configuration, system, storage system, setup, discovery 
summary: Questa procedura fornisce istruzioni dettagliate e passo passo per installare il tuo sistema storage FAS2820. 
---
= Installa il tuo sistema FAS2820 seguendo le istruzioni dettagliate
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Questa procedura fornisce istruzioni dettagliate e passo passo per la preparazione, l'installazione, il cablaggio e l'accensione del sistema storage FAS2820. Usa questi passaggi se preferisci spiegazioni approfondite invece della guida rapida abbreviata.
...
+
.. Esamina i requisiti e le fasi della procedura.
.. Raccogli informazioni sul tuo sistema storage compilando il foglio di configurazione.
...
NOTE: Se il tuo sistema non dispone di ripiani esterni, consulta link:../sas3/install-cabling-worksheets-examples-fas2600.html#fas2820-platform-in-a-multipath-ha-configuration-with-no-external-shelves["Istruzioni di cablaggio per FAS2820 in una configurazione HA multipath senza ripiani esterni"].
...
Completa la configurazione del sistema storage utilizzando l'Opzione 1: se il rilevamento di rete è abilitato, oppure l'Opzione 2: se il rilevamento di rete non è abilitato.
...
--
Se la rilevazione della rete è abilitata sul tuo laptop, completa la configurazione del sistema storage utilizzando la rilevazione automatica del cluster.

.Fasi
. Accendi l'alimentazione dei ripiani e imposta gli ID dei ripiani utilizzando l'animazione all'inizio di questo passaggio.
...
NOTE: L'avvio iniziale potrebbe richiedere fino a otto minuti.
...
. Usa la figura o i passaggi per individuare il nodo del sistema storage da configurare:
+
image::../media/drw_autodiscovery_controler_select_ieops-1849.svg[Sistema di rilevamento automatico]

+
.. Aprire file Explorer.
.. Fai clic con il pulsante destro del mouse su *Network* nel riquadro di sinistra e seleziona *refresh*.
...
+
.. Seleziona https://mysupport.netapp.com["mysupport.netapp.com"]
.. Seleziona _Crea account_ se devi creare un account o accedere al tuo account.
...
.. Configura il sistema storage utilizzando i dati raccolti in <<Fase 1: Preparazione per l'installazione>>.
