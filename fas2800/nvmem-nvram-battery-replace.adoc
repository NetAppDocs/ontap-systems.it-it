...
sidebar: sidebar 
keywords: fas2820, replace, nvmem, battery, shut, down, impaired, controller, controller, system, aggregates, metrocluster, configuration, reinstall 
summary: Per sostituire una batteria NVMEM nel tuo sistema FAS2820, devi rimuovere il modulo controller, aprirlo, sostituire la batteria, quindi richiudere e reinstallare il modulo controller. 
---
= Sostituisci la batteria NVMEM nel tuo sistema FAS2820
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Per sostituire una batteria NVMEM nel tuo sistema FAS2820, devi rimuovere il modulo controller, aprirlo, sostituire la batteria, quindi richiudere e reinstallare il modulo controller.

Se altri componenti del sistema non funzionano correttamente, contatta  https://mysupport.netapp.com/site/global/dashboard["Supporto NetApp"].
...
NOTE: Il LED verde NV sul pannello frontale inizia a lampeggiare quando l'alimentazione viene interrotta se il sistema era nello stato "in attesa di giveback" o se il sistema non è stato sottoposto a takeover o arrestato correttamente (dati non confermati). Se il modulo controller danneggiato non è stato sottoposto correttamente a takeover dal partner controller, contatta https://mysupport.netapp.com/site/global/dashboard["Supporto NetApp"]

+
** Se il LED di stato NV verde inizia a lampeggiare quando il modulo controller viene rimosso dal telaio:
+
*** Conferma che il controller abbia subito un takeover pulito dal partner controller, oppure che il controller danneggiato mostri _waiting for giveback_. Se è così, puoi ignorare il LED lampeggiante e continuare a rimuovere il controller danneggiato dallo chassis.


** Se il LED verde NV è spento, puoi completare la rimozione del controller difettoso dallo chassis.
...
.Fasi
. Se non si è già collegati a terra, mettere a terra l'utente.
. Rimuovi il modulo controller dallo chassis.
...
. Installa la batteria di ricambio:
+
