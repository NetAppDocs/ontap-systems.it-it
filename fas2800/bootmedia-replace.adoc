...
summary: 'Il boot media nel tuo sistema FAS2820 memorizza i dati essenziali del firmware e della configurazione. Per sostituirlo, devi rimuovere e aprire il modulo controller danneggiato, individuare e sostituire il boot media, trasferire l"immagine di boot su una chiavetta USB e poi avviare il controller.' 
---
= Sostituisci il supporto di boot nel tuo sistema FAS2820 utilizzando la procedura di ripristino manuale del boot
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Il boot media nel tuo sistema FAS2820 memorizza i dati essenziali del firmware e della configurazione. Per sostituirlo, devi rimuovere e aprire il modulo controller danneggiato, individuare e sostituire il boot media, trasferire l'immagine di boot su una chiavetta USB, inserire la chiavetta USB nel controller e poi avviare il controller.
...
Individua il supporto di avvio, che si trova sotto la scheda mezzanine nel modulo controller, e segui le istruzioni per sostituirlo.
...
+
.. Usa  https://kb.netapp.com/on-prem/ontap/DM/Encryption/Encryption-KBs/How_to_determine_if_the_running_ONTAP_version_supports_Data_At_Rest_Encryption["Come determinare se la tua versione di ONTAP supporta la NetApp Volume Encryption (NVE)"^] per determinare se la crittografia del volume è attualmente supportata.
+
