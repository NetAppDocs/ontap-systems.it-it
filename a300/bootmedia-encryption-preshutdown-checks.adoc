...
[role="lead"]
Per garantire la sicurezza dei dati sul sistema di archiviazione, è necessario verificare il supporto e lo stato della chiave di crittografia sul supporto di avvio. Il sistema AFF A300 supporta solo procedure di ripristino manuale tramite supporti di avvio. Il ripristino automatico del supporto di avvio non è supportato.

Verificare se la versione ONTAP supporta NetApp Volume Encryption (NVE) e, prima di arrestare il controller, verificare che il gestore delle chiavi sia attivo.
