...
summary: Per ripristinare il sistema AFF A700s alla piena operatività, ripristina la configurazione di NetApp Storage Encryption se necessario, installa le licenze per il nuovo controller e restituisci la parte guasta a NetApp. 
---
= Completa la sostituzione del controller per il tuo sistema AFF A700s
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Completa la sostituzione del controller per il tuo sistema AFF A700s ripristinando la configurazione di crittografia dello storage NetApp (se necessario), installando le licenze per il nuovo controller e restituendo il componente guasto a NetApp.
