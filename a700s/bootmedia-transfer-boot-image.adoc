...
summary: 'Trasferire l"immagine di avvio sul supporto di avvio sostitutivo su un sistema AFF A700s utilizzando il supporto di avvio secondario o un"unità flash USB. Il sistema AFF A700s supporta solo procedure di ripristino manuale tramite supporti di avvio. Il ripristino automatico del supporto di avvio non è supportato.' 
---
= Trasferisci l'immagine di avvio sul supporto di avvio del tuo sistema AFF A700s utilizzando una chiavetta USB
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Trasferire l'immagine di avvio sul supporto di avvio sostitutivo di un AFF A700s utilizzando il supporto di avvio secondario (metodo principale) o un'unità flash USB (se il ripristino del supporto di avvio secondario non riesce o il  `image.tgz` file è mancante).
...
== Opzione 1: Trasferire l'immagine di sistema utilizzando il ripristino di backup dal secondo supporto di avvio

È possibile installare l'immagine di sistema sul supporto di avvio sostitutivo utilizzando l'immagine presente sul secondo supporto di avvio nel modulo controller. Questo è il metodo principale per i sistemi con due supporti di avvio nel modulo controller.

L'immagine sul supporto di avvio secondario deve contenere un  `image.tgz` file e non deve segnalare errori. Se il  `image.tgz` file è mancante o il supporto di avvio segnala errori, non è possibile utilizzare questa procedura. Utilizzare <<Opzione 2: trasferire l'immagine di avvio tramite un'unità flash USB>> invece.
...
== Opzione 2: trasferire l'immagine di avvio tramite un'unità flash USB

Questa procedura deve essere utilizzata solo se il ripristino del supporto di avvio secondario non è riuscito o se il `image.tgz` file non viene trovato sul supporto di avvio secondario.

* È necessario disporre di un'unità flash USB, formattata con FAT32, con almeno 4 GB di capacità.
* Una copia della stessa versione dell'immagine di ONTAP utilizzata dal controller difettoso. È possibile scaricare l'immagine appropriata dalla sezione Download del NetApp Support Site.
+
...
.. Salva le variabili d'ambiente che hai modificato con il comando  `savenv`.
...
. Dal controller partner nel livello di privilegio avanzato, avviare la sincronizzazione della configurazione utilizzando l'indirizzo IP registrato nel passaggio precedente: `system node restore-backup -node local -target-address _impaired_node_IP_address_`
. Una volta completata la sincronizzazione della configurazione senza errori, premere `y` quando viene richiesto di confermare che la procedura di backup è stata eseguita correttamente.
. Verificare che le variabili d'ambiente siano impostate come previsto.
+
