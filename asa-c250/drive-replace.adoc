...
summary: 'Sostituisci a caldo un"unità nel tuo sistema ASA C250 quando la piattaforma registra un messaggio di avviso nella console di sistema che indica che un"unità ha avuto un guasto. Inoltre, sia il LED di errore sul pannello di controllo dell"operatore sia il LED di errore sull"unità guasta sono accesi.' 
---
= Sostituisci a caldo un disco in un sistema ASA C250
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sostituisci a caldo un'unità nel tuo sistema ASA C250 quando un'unità si guasta. La procedura di sostituzione prevede l'identificazione dell'unità difettosa, la rimozione in sicurezza e l'installazione di una nuova unità per garantire la continuità dell'accesso ai dati e le prestazioni del sistema.
