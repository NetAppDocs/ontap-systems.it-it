---
permalink: asa-c250/install-setup.html 
sidebar: sidebar 
keywords: asa c250, install, setup, initial configuration 
summary: Installa e configura un sistema storage ASA C250 usando passaggi rapidi, video o passaggi dettagliati. 
---
= Flusso di lavoro per l'installazione e la configurazione di ASA C250
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Installa e configura un sistema storage ASA C250 usando passaggi rapidi, video o passaggi dettagliati.

* link:../asa-c250/install-quick-guide.html["Passaggi rapidi"]
+
Un PDF stampabile con istruzioni dettagliate con collegamenti in tempo reale a contenuti aggiuntivi.

* link:../asa-c250/install-videos.html["Video fasi"]
+
Video istruzioni dettagliate.

* link:../asa-c250/install-detailed-guide.html["Passaggi dettagliati"]
+
Istruzioni passo-passo online con collegamenti in tempo reale a contenuti aggiuntivi.



Se il sistema si trova in una configurazione MetroCluster IP, consultare https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/index.html["Installare la configurazione IP di MetroCluster"^] istruzioni.
