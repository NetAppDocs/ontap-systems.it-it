...
summary: Per ripristinare il sistema AFF A900 al funzionamento completo, ricablare lo storage, ripristinare la configurazione NetApp Storage Encryption se necessario e installare le licenze per il nuovo controller. 
---
= Completa la sostituzione del controller per il tuo sistema AFF A900
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Per completare la sostituzione del controller per il tuo sistema AFF A900, ripristina la configurazione di NetApp Storage Encryption (se necessario), verifica che le interfacce logiche (LIF) stiano riportando alle loro porte principali, esegui un controllo dello stato del cluster e restituisci la parte guasta a NetApp.
