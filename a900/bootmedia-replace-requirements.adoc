...
summary: Prima di sostituire il supporto di boot nel tuo sistema AFF A900, verifica di soddisfare i requisiti necessari per una sostituzione corretta. Questo include assicurarti di avere una chiavetta USB con la capacità di storage adeguata e di avere il dispositivo di boot sostitutivo corretto. 
---
= Esamina i requisiti per il ripristino manuale del supporto di avvio nel tuo sistema AFF A900
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Dopo aver esaminato i requisiti per la sostituzione del supporto di boot, link:bootmedia-encryption-preshutdown-checks.html["controllare il supporto e lo stato della chiave di crittografia sul supporto di avvio"].
