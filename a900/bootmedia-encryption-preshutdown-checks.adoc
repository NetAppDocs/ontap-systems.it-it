...
[role="lead"]
Per garantire la sicurezza dei dati sul sistema storage AFF A900, è necessario verificare il supporto e lo stato della chiave di crittografia sul supporto di avvio. Controlla se la versione di ONTAP supporta NetApp Volume Encryption (NVE). Prima di spegnere il controller, controlla se il key manager è attivo.
