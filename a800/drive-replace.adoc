...
summary: 'Devi sostituire a caldo un"unità quando la piattaforma registra un messaggio di avviso sulla console di sistema che indica che un"unità ha avuto un guasto. Inoltre, sia il LED di errore sul pannello di controllo dell"operatore sia il LED di errore sull"unità guasta sono accesi.' 
---
= Sostituisci a caldo un disco in un sistema AFF A800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sostituisci a caldo un'unità nel tuo sistema AFF A800 quando un'unità si guasta o necessita di un aggiornamento. Il processo di sostituzione prevede l'identificazione dell'unità guasta, la sua rimozione in sicurezza e l'installazione di una nuova unità per garantire la continuità dell'accesso ai dati e le prestazioni del sistema.
