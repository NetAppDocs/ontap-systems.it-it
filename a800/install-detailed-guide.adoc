...
---
= Installa il tuo sistema AFF A800 seguendo istruzioni dettagliate passo passo
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Installa il tuo sistema storage AFF A800 seguendo queste istruzioni dettagliate passo passo, dalla preparazione del sito all'avvio del cluster ONTAP.
...
È necessario effettuare il cablaggio richiesto per il cluster della tua piattaforma utilizzando il metodo cluster senza switch a due nodi o il metodo rete di interconnessione del cluster. C'è un cablaggio opzionale verso le reti host Fibre Channel o iSCSI o verso lo storage collegato direttamente. Questo cablaggio non è esclusivo; puoi avere il cablaggio sia verso una rete host che verso lo storage.
