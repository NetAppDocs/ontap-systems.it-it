...
---
= Ricollega i cavi del sistema e riassegna i dischi nel tuo sistema AFF A800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Ricollega i cavi del sistema AFF A800 dopo la sostituzione di un controller e riassegna i dischi al controller sostitutivo. Se il tuo sistema è in una coppia HA, il processo di giveback assegna automaticamente l'ID del sistema ai dischi; i passaggi esatti dipendono dal fatto che il sistema utilizzi nessuna crittografia o la crittografia Onboard Key Manager (OKM).
...
Se il sistema storage è in una coppia HA, l'ID di sistema del nuovo modulo controller viene assegnato automaticamente ai dischi quando avviene il giveback alla fine della procedura. Devi confermare la modifica dell'ID di sistema quando avvii il controller di _replacement_ e poi verificare che il sistema abbia applicato la modifica.
