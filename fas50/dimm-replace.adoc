...
summary: Sostituisci il DIMM nel tuo sistema storage FAS50 se vengono rilevati errori di memoria correggibili o non correggibili in eccesso. Tali errori possono impedire al sistema storage di avviare ONTAP. La procedura di sostituzione prevede lo spegnimento del controller guasto, la sua rimozione, la sostituzione del DIMM, la reinstallazione del controller e poi la restituzione del componente guasto a NetApp. 
---
= Sostituisci un modulo DIMM in un sistema FAS50 per risolvere gli errori di memoria
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
