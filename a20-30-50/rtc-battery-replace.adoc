...
summary: 'Sostituisci la batteria dell"orologio in tempo reale, comunemente nota come batteria a bottone, nel tuo sistema storage AFF A20, AFF A30 o AFF A50 per garantire che i servizi e le applicazioni che si basano su una sincronizzazione oraria precisa rimangano operativi.' 
---
= Sostituisci la batteria dell'orologio in tempo reale nel tuo sistema AFF A20, AFF A30 o AFF A50
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Sostituisci la batteria dell'orologio in tempo reale (RTC), comunemente nota come batteria a bottone, nel tuo sistema di archiviazione AFF A20, AFF A30 o AFF A50 per garantire che i servizi e le applicazioni che si basano su una sincronizzazione temporale accurata rimangano operativi.

.Prima di iniziare
Tutti gli altri componenti del sistema storage devono funzionare correttamente. Se non è così, contatta https://mysupport.netapp.com/site/global/dashboard["Supporto NetApp"] prima di continuare con questa procedura.
...
|===
. Ruota la vite a testa zigrinata in senso antiorario per allentarla, quindi apri il coperchio del controller.
...
NOTE: Dopo aver sostituito la batteria RTC, inserito il controller e acceso il sistema per il primo ripristino del BIOS, vengono visualizzati i seguenti messaggi di errore:
`RTC date/time error. Reset date/time to default`
`RTC power failure error` Questi messaggi sono previsti e puoi continuare con questa procedura.
...
NOTE: Se il sistema storage si blocca nel menu di boot, seleziona l'opzione  `Reboot node` e rispondi _y_ quando richiesto, poi avvia il LOADER premendo _Ctrl-C_.
...
. Al prompt LOADER, inserisci `bye` per reinizializzare i moduli I/O e gli altri componenti e per riavviare il controller.
