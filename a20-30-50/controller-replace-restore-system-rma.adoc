...
[role="lead"]
Ripristinare la configurazione della crittografia dello storage NetApp (se necessario), installare le licenze per il controller sostitutivo, verificare che le interfacce logiche (LIF) riportino alla porta home, eseguire un controllo dello stato del cluster, registrare il numero di serie del controller sostitutivo e restituire il componente guasto a NetApp per completare l'ultimo passaggio della procedura di sostituzione del controller AFF A20, AFF A30 o AFF A50.
