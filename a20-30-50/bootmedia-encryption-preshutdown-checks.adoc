...
summary: Per garantire la sicurezza dei dati sul sistema di storage A20, AFF A30 o AFF A50, è necessario verificare il supporto della chiave di crittografia e lo stato del supporto di avvio. Controllare se la versione di ONTAP in uso supporta la crittografia dei volumi NetApp e prima di arrestare il controller verificare se il gestore delle chiavi è attivo. 
---
= Verifica il supporto della crittografia sul supporto di avvio in un sistema AFF A20, AFF A30 o AFF A50 utilizzando la procedura di ripristino manuale all'avvio
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Per garantire la sicurezza dei dati sul tuo sistema storage AFF A20, AFF A30 o AFF A50, devi verificare il supporto e lo stato della chiave di crittografia sul tuo supporto di boot. Controlla se la tua versione di ONTAP supporta NetApp Volume Encryption (NVE). Prima di spegnere il controller, controlla se il key manager è attivo.
