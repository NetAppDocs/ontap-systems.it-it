...
---
= Accendere il sistema di archiviazione - AFF C80
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Dopo aver installato l'hardware rack per il sistema di storage AFF C80 e aver installato i cavi per i controller e gli shelf di storage, è necessario accendere gli shelf e i controller di storage.
...
* Un ID ripiano esterno valido è compreso tra 01 e 99.
+
...
.. Premere e rilasciare il pulsante ID ripiano per far avanzare il numero fino a raggiungere il numero desiderato da 0 a 9, o da 1 a 9 se il sistema dispone di spazio di archiviazione integrato.
+
...
Dopo aver acceso il sistema di archiviazione AFF C80, si link:install-complete.html["completare la configurazione del sistema"].
